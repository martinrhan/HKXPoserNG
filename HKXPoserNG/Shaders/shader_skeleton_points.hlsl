...
void GS(point VS_OUT vin[1], inout TriangleStream<GS_OUT> outStream)
{
    ...
    output00.position = center + float4(-.005, -.005, 0, 0);
    output01.position = center + float4(-.005, .005, 0, 0);
    output10.position = center + float4(.005, -.005, 0, 0);
    output11.position = center + float4(.005, .005, 0, 0);
